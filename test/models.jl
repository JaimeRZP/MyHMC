@testset "Models" begin
    @testset "Gaussian" begin
        ...
        spl = MyHMC(10_000, 0.01; init_eps = sqrt(d))
        samples_MyHMC = Sample(spl, target, 100_000; dialog = true)
        samples_MyHMC_adaptive =
            Sample(spl, target, 100_000; adaptive = true, dialog = true)

        _samples_MyHMC = mapreduce(permutedims, vcat, samples_MyHMC)
        s1 = std(_samples_MyHMC, dims = 1)[1:end-3]
        m1 = mean(_samples_MyHMC, dims = 1)[1:end-3]

        _samples_MyHMC_adaptive = mapreduce(permutedims, vcat, samples_MyHMC_adaptive)
        s2 = std(_samples_MyHMC_adaptive, dims = 1)[1:end-3]
        m2 = mean(_samples_MyHMC_adaptive, dims = 1)[1:end-3]
    ...
    @testset "Rosembrok" begin
        ...
        spl = MyHMC(10_000, 0.01; L = sqrt(2), sigma = ones(target.d), adaptive = true)
